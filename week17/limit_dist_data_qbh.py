...
def plot_data_histogram(fits, name):
    ...
    label.DrawText(0.5, 0.7, "{0}GeV qbh".format(MASS_FILE))
...
def fit_data_likelihood(simulation_mass):
    root_file_data_old = TFile.Open("data/mjj_data15_13TeV_00276262_physics_Main_total_final.root")
    ...
    root_file_qstar = TFile.Open("data/dataLikeHistograms.BlackMax{0}.root".format(simulation_mass))
    nominal = root_file_qstar.GetDirectory("Nominal")
    hist_model = nominal.Get("mjj_Scaled_BlackMax{0}_30fb".format(simulation_mass))
    hist_model.Smooth(1)
    hist_model.Scale(1/hist_model.Integral())
...
def data_confidence_dist():
    ...
    with open("results/data_cl_dist_qbh.txt", "w") as out_file:
        for mass in [4000, 5000, 5500, 6000, 6500, 7000, 7500, 8000, 8500, 9000, 9500, 10000]:
