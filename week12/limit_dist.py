...
class Fits:
    ...
    def run_mass_fit(self, peak_scale_initial):
        ...
        while N < 5000:
            ...
            end_sum = sum([math.exp(-a) for a in y_values])

            # if new value changes total by less than 0.01%, end loop
            if N > 1000 and (end_sum-start_sum)/start_sum < 0.0001:
...
def plot_data_histogram(fits, name):
    ...
    test_canvas.SaveAs("plots/{0}/plot-{1}-{2}-hist.png".format(CLUSTER_ID, name, MASS_FILE))
...
def fit_significance(num_injected_events, plot=True, name=""):
    ...
    label.SetTextSize(0.03)
    label.DrawText(0.5, 0.7, "{0}GeV q*".format(MASS_FILE))
    canvas.SaveAs("plots/{0}/plot-{1}-{2}-sig_dist.png".format(CLUSTER_ID, name, MASS_FILE))
    ...
    canvas2.SaveAs("plots/{0}/plot-{1}-{2}-sig_cumsum.png".format(CLUSTER_ID, name, MASS_FILE))
...
def plot_95pc_confidence_dist():
    ...
    with open("results/{0}/{1}/job-{2}-{1}.txt".format(CLUSTER_ID, MASS_FILE, JOB_NAME), "w") as out_file:
        while successes < want_successes and total_iterations < 2*want_successes:
            ...
            print("Iteration {0}, limit = {1}".format(total_iterations, limit))
            if limit is None:
                continue
            out_file.write("{0}\n".format(limit))
            successes += 1
