...
void FitExample11(){
    ...
    // I have fixed the mean location and width of the peak in these functions. Only one parameter in Gaussin Peak
    ...
    TCanvas *TestCanvas = new TCanvas ("TestCanvas","Fitting Test 11",0,0,1200,800);
    ...
    // The signal will also be over the range 0.->200., with 2 parameters, mean fixed
    ...
    comb->SetParLimits(3,50.,50.); // Fix mean to 50.
    comb->SetParameter(4,5.); // Width = 5.
    comb->SetParLimits(4,5.,5.); // Fix width to 5.
    ...
    TestCanvas->Print("Fit_Example_11.pdf");
    TestCanvas->Print("Fit_Example_11.gif");
