...
void FitExample5(){
  ...
  TFile *hfile = new TFile("Test_Fitting_5.root","old","Exercise 1");
  ...
  TestCanvas->Print("Fit_Example_5.ps");
  TestCanvas->Print("Fit_Example_5.gif");
  
  
  // Get fit parameters into variables
  TF1 *fit = h_test_1->GetFunction("gaus");
  ...
  cout << " The fitted mean     value = " << p[1] << " +/- " << e[1] << endl;
  cout << " The fitted sigma    value = " << p[2] << " +/- " << e[2] << endl;
  
  // Now we will fit with a function
  
  
  // define a new function, gaussian + constant 
  TF1 *my_func = new TF1("my_func","gaus+[3]",0,50);
  
  // set the initail values of the function to the fit values plus a constant of 50. 
  
  my_func->SetParameters(p[0],p[1],p[2],50.);
  ...
  TestCanvas->Print("Fit_Example_5_new.pdf");
  TestCanvas->Print("Fit_Example_5_new.gif");
