...
def fit_significance(num_injected_events, plot=True, name=""):
    ...
    label.SetNDC()
    label.SetTextSize(0.03)
    label.DrawText(0.5, 0.7, "{0}GeV WPrime".format(MASS_FILE))
...
def fit_data_likelihood(simulation_mass):
    ...
    root_file_qstar = TFile.Open("data/dataLikeHistograms.WPrime{0}.root".format(simulation_mass))
    nominal = root_file_qstar.GetDirectory("Nominal")
    hist_model = nominal.Get("mjj_Scaled_WPrime{0}_30fb".format(simulation_mass))
    hist_model.Smooth(1)
    hist_model.Scale(1/hist_model.Integral())
...
def data_confidence_dist():
    ...
    with open("results/data_cl_dist_wprime.txt", "w") as out_file:
        for mass in range(2000, 6501, 500):
