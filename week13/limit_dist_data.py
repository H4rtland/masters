...
def fit_data_likelihood(simulation_mass):
    root_file_data = TFile.Open("data/mjj_data15_13TeV_00276262_physics_Main_total_final.root")
    hist_data = root_file_data.Get("mjj_data15_13TeV_00276262_physics_Main_total_final")

    hist_bins = hist_data.GetNbinsX()
    ...
    root_file_qstar = TFile.Open("data/dataLikeHistograms.QStar{0}.root".format(simulation_mass))
    nominal = root_file_qstar.GetDirectory("Nominal")
    ...
    if fits.iflag != 4:
        print("Fit did not converge mass={0}".format(mass))
        return None
    ...
    ycumulative = [sum(y[0:i]) for i in range(0, len(y))]
    if max(ycumulative) == 0:
        print("Iteration mass={0} max(ycumulative)=0".format(mass))
        plot_data_histogram(fits, name)
        return None
    ycumulative = [yval/max(ycumulative) for yval in ycumulative]
    limit_x = 0
...
def data_confidence_dist():
    ...
    with open("results/data_cl_dist.txt", "w") as out_file:
